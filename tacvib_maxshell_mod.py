...
for subject in get_subject_ids(get_subjects(paradigm_dir)):
    ...
    if proc_using_erm: # maxwell filter ERM data if specified, is there a better way to handle this case?
        date_paradigm = read_measure_date(raw.info) # match recording date to find the correct file
        replace_dict = {'subject': subject, 'paradigm': paradigm, 'date': date_paradigm}
