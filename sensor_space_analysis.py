...
def main(subject, subject_fnames, log):
    ...
    for condition_name, condition_info in para_cfg.conditions_dicts.items(): # read epochs around conditions/event IDs
        ...
        power, itc = anal.calc_sensor_tfr(epochs, para_cfg.freqs, para_cfg.n_cycles, para_cfg.n_jobs,
                                           subject_fnames['epochs_sensor_subdir'], sensor_tfr_fname)

        # plot, save TFR figure(s) that we time-window
        anal.analyze_sensor_tfr(itc, para_cfg.tfr_temporal_dict, subject_fnames['epochs_sensor_subdir'], sensor_tfr_plot_fname, para_cfg.eeg)

        # plot, save evoked sensor data
        vis.plot_evoked_sensor(epochs, subject_fnames['epochs_sensor_subdir'], subject_fnames['evoked_plot'])
